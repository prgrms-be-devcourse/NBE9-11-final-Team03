...
> 기준일: 2026-06-22  
> 기준 브랜치: `dev`  
> 기준 PR: `#62`, `#63`, `#64` 병합 완료 / `#67`, `#68` 진행 중  
> 문서 상태: 최신 구현 기준 요약  
> 프로젝트: Baton  
> 팀: Team3  
...
Baton은 돈이 부족한 주니어가 자신의 재능을 기반으로 필요한 도움을 얻을 수 있도록 돕는 크레딧 기반 재능 거래/교환 서비스다. 발표와 제출 문서의 중심은 `PURCHASE MVP`이며, SWAP과 리뷰, 채팅, 관리자 기능은 MVP 이후 확장 흐름으로 분리한다.
...
| MVP 중심 | 회원가입/로그인부터 PURCHASE 거래, 에스크로, 정산, 크레딧 이력까지의 흐름 | 구현 최종 검증 필요 |
...
| SWAP 확장성 | PURCHASE 안정화 이후 양방향 재능 교환으로 확장 | 후순위 |
| 신뢰 점수/리뷰 | 거래 품질과 사용자 신뢰를 누적 | 후순위 |
...
| 초기 크레딧 | `CreditService.initializeAccount` | 서비스 구현, 회원가입 자동 연동 TODO 확인 | 높음 |
| 재능 등록/조회 | `TalentController` | 구현 확인 | 낮음 |
| 매칭 추천 | `MatchRecommendationController` | 구현 확인 | 낮음 |
| 매칭 제안 생성/수락/거절 | `MatchProposalController`, `MatchProposalService` | 수락 시 Trade/Credit/Escrow 연결 구현 | 신규 계좌 없으면 hold 실패 |
...
| 크레딧 예치/환불/이력 | `CreditService`, `CreditTransaction` | 서비스/엔티티 구현 | 조회 API 부재 |
| 거래 완료/정산 | `TradeSubmissionService.confirmPurchase`, `CreditService.settleEscrow` | 구현 확인 | 통합 시연 필요 |
...
| Credit | 잔액 조회, 초기 계좌 생성, 적립, 차감, 에스크로 보류, 환불, 정산, 이력 기록 | 초기 계좌 자동 연결/이력 조회 방식 검증 필요 |
| Escrow | 에스크로 생성, 환불, 정산 상태 관리 | 매칭 수락/구매 확정 흐름에 연결 |
| Trade | 거래 생성, 조회, 취소, 결과물 제출, 구매 확정 | 수락 연동 구현, 통합 시연 필요 |

### P1: MVP 이후 우선 개발 후보

| 기능 | 발표 포지션 | 상태 |
|---|---|---|
| 관리자 기능 | 운영 안정성 확장 | 후순위 |
| SWAP 거래 | Baton의 장기 차별화 기능 | 후순위 |
| 리뷰/신뢰 점수 | 거래 신뢰도 강화 | 일부 필드 기반 존재, 정책/API 검증 필요 |
| 채팅 | 거래 전후 커뮤니케이션 보강 | REST 기반 채팅방/메시지 API 존재, 실시간 Pub/Sub는 고도화 |
...
| 리스크 | 영향 | 대응 |
|---|---|---|
| PURCHASE 전체 흐름 통합 시연 미검증 | 핵심 시연 실패 가능 | 최종 API 흐름을 Swagger/Postman으로 재검증 |
| 최신 테스트 2건 실패 | 품질 지표 신뢰도 하락 | 쿠키 Secure 옵션 테스트 또는 설정 정리 |
...
| 우선순위 | 해야 할 일 | 완료 기준 | 넘길 채팅/담당 |
|---|---|---|---|
| P0 | PURCHASE E2E 통합 테스트 추가 | 매칭 제안 수락부터 에스크로 보류, 제출, 구매 확정, 정산까지 한 흐름으로 검증된다. | 백엔드 / QA |
| P1 | CreditTransaction 조회 API PR 리뷰/머지 | `/api/v1/credit/transactions`가 dev에 병합되어 Swagger/Postman 시연에서 크레딧 이력을 확인할 수 있다. | 백엔드 / QA |
| P1 | SecurityConfig 인증/CORS PR 리뷰/머지 | 로그인/회원가입 제외 API 인증 정책과 CORS 설정이 dev에 병합된다. | 백엔드 / 배포 |
