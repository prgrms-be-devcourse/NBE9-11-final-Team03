...
| MVP 중심 | 회원가입/로그인부터 PURCHASE 거래, 에스크로, 정산, 크레딧 이력까지의 흐름 | 정상 흐름 검증 완료, 재조회 상태 이슈 확인 필요 |
...
| 거래 완료/정산 | `TradeSubmissionService.confirmPurchase`, `CreditService.settleEscrow` | 구현/정산 검증 | 구매 확정 후 상세 재조회 상태 불일치 |
...
| Trade | 거래 생성, 조회, 취소, 결과물 제출, 구매 확정 | 정상 흐름 검증 완료, 구매 확정 후 재조회 상태 확인 필요 |
...
| 리스크 | 영향 | 대응 |
|---|---|---|
| 구매 확정 후 거래 재조회 상태 불일치 | 완료 상태 시연 실패 가능 | `confirm` 이후 `GET /trade/{id}`가 `COMPLETED/RELEASED`인지 수정/재검증 |
...
| 우선순위 | 해야 할 일 | 완료 기준 | 넘길 채팅/담당 |
|---|---|---|---|
| P0 | 구매 확정 후 거래 재조회 상태 불일치 해결 | confirm 응답과 trade detail 응답이 모두 `COMPLETED/RELEASED`다. | 백엔드 / QA |
...
| 우선순위 | 해야 할 일 | 완료 기준 | 넘길 채팅/담당 |
|---|---|---|---|
| P0 | 구매 확정 후 거래 상세 재조회 상태 불일치 해결 | `GET /api/v1/trade/{tradeId}` 재조회도 `COMPLETED/RELEASED`로 응답한다. | 백엔드 / QA |
| P1 | 실패 케이스 테스트 | 크레딧 부족, 중복 수락, 권한 없음, 재확정, idempotency 재사용이 검증된다. | 백엔드 / QA |
