...
주의: 위 흐름은 발표 목표 기준의 MVP 완성 흐름이다. 현재 코드 기준으로 회원가입 후 초기 크레딧 지급은 `AuthService.signup()`에서 `CreditService.initializeAccount()`로 연결되어 있고, 매칭 수락 이후 Trade 생성, Credit 에스크로 보류, Escrow 생성은 `MatchProposalService.acceptMatchProposal`에 연결되어 있다. 다만 2026-06-23 수동 테스트에서 구매 확정 응답과 직후 거래 상세 재조회 상태가 불일치하는 이슈가 관측되어 발표 전 재검증이 필요하다.
...
| 리스크 | 대응 |
|---|---|
| 구매 확정 후 거래 재조회 상태 불일치 | `confirm` 응답과 `trade detail` 조회 결과가 모두 `COMPLETED/RELEASED`인지 발표 전 재검증 |
...
1. 구매 확정 후 거래 재조회 상태 불일치 해결 여부 반영
