...
| 5 | 테스트 코드 작성/검증 방향 확인 | [TradeSettlementPersistenceIntegrationTest](../../../src/test/java/com/back/baton/domain/trade/service/TradeSettlementPersistenceIntegrationTest.java) |
...
현재 PURCHASE MVP는 수동 API 테스트로 정상 흐름을 검증했고, 구매 확정/거래 취소 후 상태가 DB에 저장되는지 확인하는 통합 테스트를 추가했다.

작성한 테스트:

- `TradeSettlementPersistenceIntegrationTest`
- 구매 확정 후 `Trade.status=COMPLETED`, `Escrow.status=RELEASED`가 DB 재조회에서도 유지되는지 검증
- 거래 취소 후 `Trade.status=CANCELLED`, `Escrow.status=REFUNDED`가 DB 재조회에서도 유지되는지 검증
- `EntityManager.clear()` 후 다시 조회해 영속성 컨텍스트가 아니라 실제 DB 반영 여부를 확인

멘토님께 확인받고 싶은 점:

- 이 테스트가 구매 확정 상태 저장 이슈를 검증하기에 충분한지
...
| 테스트 코드 작성 내용을 설명할 때 | [TradeSettlementPersistenceIntegrationTest](../../../src/test/java/com/back/baton/domain/trade/service/TradeSettlementPersistenceIntegrationTest.java) |
