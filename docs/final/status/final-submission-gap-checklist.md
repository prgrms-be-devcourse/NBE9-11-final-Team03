...
| 최종 API 명세서 | 필수 | `docs/api/api-spec-v1.md`, Swagger | 원격 dev 기준 최신 API와 상태 전이 반영 |
...
| Postman/Swagger UI 기반 API 호출 시연 영상 | 필수 | `demo-script.md` | PURCHASE MVP 전체 흐름 녹화 |
...
추가 기능은 반드시 하더라도 최종 문서에서는 `구현 완료`, `부분 완료`, `확장 계획`을 분리해야 한다.
...
| 채팅 | 채팅방 생성과 메시지 송수신이 가능하면 구현 완료 |
...
| 항목 | 담당/확인 | 완료 기준 |
| --- | --- | --- |
| 회원가입 후 초기 크레딧 자동 지급 연결 | 백엔드 | `AuthService.signup()` 이후 `CreditService.initializeAccount(userId)` 호출 및 테스트 |
| CreditTransaction 조회 API 여부 결정 | 팀장 | Swagger/Postman 시연에서 이력 확인 방식을 확정 |
| PURCHASE E2E 통합 테스트 | 백엔드 | 가입 또는 계좌 준비부터 구매 확정/정산까지 한 테스트로 검증 |
...
| 항목 | 담당/확인 | 완료 기준 |
| --- | --- | --- |
| 회원가입 후 초기 크레딧 자동 지급 PR 확인 | 백엔드 | PR #62 머지 후 `AuthService.signup()`에서 `CreditService.initializeAccount(userId)` 호출 확인 |
| 진행 중 거래 재능 삭제 차단 PR 수정 확인 | 백엔드 | PR #64에서 `IN_PROGRESS`뿐 아니라 `UNDER_REVIEW` 등 완료 전 상태 차단 |
| PURCHASE E2E 통합 테스트 | 백엔드 | 가입 또는 계좌 준비부터 구매 확정/정산까지 한 테스트로 검증 |
...
| CreditTransaction 조회 API 여부 결정 | 방향 확정 | PR #67에서 `/api/v1/credit/transactions` 구현 진행 중 |
