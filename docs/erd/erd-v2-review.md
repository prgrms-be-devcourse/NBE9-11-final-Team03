...
|---|---|---|---|
| 1 | 사용자 가입 | `users` | 구현 |
| 2 | 크레딧 계좌 생성 | `credit_account` | 서비스 존재, 회원가입 자동 연동 TODO |
...
| 11 | 크레딧 거래 이력 기록 | `credit_transaction` | 기록 구현, 조회 API 확인 필요 |

현재 `MatchProposal` 수락 API는 상태 변경뿐 아니라 Trade 생성, 크레딧 에스크로 보류, Escrow 생성을 함께 수행한다.  
다만 회원가입 직후 CreditAccount 자동 생성은 아직 연결되지 않은 P0 잔여 작업이다.
...
- [ ] 실제 FK와 논리 FK를 구분했다.
- [ ] 회원가입 후 초기 크레딧 자동 지급 미연결 리스크를 표시했다.
